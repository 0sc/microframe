...
class FormHelperTest < Minitest::Test
  ...
  def test_start_form_without_default
    assert_equal "<form action='link_to_sample' method='post'>", @help.start_form
  ...
  def test_start_form_with_default
    ...
    refute_equal "<form action='link_to_sample' method='post'>", @help2.start_form
  ...
  def test_submit_without_start_of_form
    assert_equal "<form action='link_to_sample' method='post'><input type = 'submit' value = 'save' /></form>", @help.submit
  end

  def test_submit_with_start_of_form
    @help.start_form
    assert_equal "<input type = 'hidden' name = '_method' value = 'put'/><input type = 'submit' value = 'save' /></form>", @help.submit
  end
  ...
  def test_hidden_field_without_start_of_form
    assert_equal "<form action='link_to_sample' method='post'><input type = 'hidden' name = 'sample_id' value = '123'/>", @help.hidden(Sample.new)
  end

  def test_hidden_field_with_start_of_form
    @help.start_form
    assert_equal "<input type = 'hidden' name = 'sample_id' value = '123'/>", @help.hidden(Sample.new)
  end

  def test_gatekeeper
    output = "output"
    assert_equal "<form action='link_to_sample' method='post'>#{output}", @help.gatekeeper(output)
